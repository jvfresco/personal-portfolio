...
function reducer(state, action) {
  switch (action.type) {
    ...
    case "OPEN_MODAL":
      return { ...state, index: action.index, showModal: true };
    case "UNLOAD_PROJECT":
...
const App = () => {
  const [{ showModal, animate, index }, dispatch] = useReducer(reducer, {
    showModal: false,
    animate: false,
    index: null,
  });
        ...
        >
          <span
            dangerouslySetInnerHTML={{
              __html: index !== null && projects[index].text,
            }}
          />
          <div
          ...
          >
            <IconLink
              href={index !== null && projects[index].urlGithub}
              image={githubLogo}
              alt="Visit on Github"
              color={index !== null && projects[index].colors.filter}
            />

            {
              <IconLink
                href={index !== null && projects[index].urlLive}
                image={index === projects?.length - 1 ? mailLogo : liveLogo}
                alt="Visit live version"
                color={index !== null && projects[index].colors.filter}
              />
            }
          </div>
