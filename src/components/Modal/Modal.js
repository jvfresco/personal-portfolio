...
const Modal = forwardRef(({show, modalClosed, children, colors, title}, ref) => {
      ...
      <Backdrop show={show} clicked={modalClosed} />
      {
        animations.map(({item, key, props}) => {
          ...
          <animated.div key={key} style={{...props}} className={"modal"}>
            <div className='modal__title' style={{backgroundColor: background, color: colors.secondary ? colors.secondary : '#dfdfdf'}}>
                {title ? title : 'About me'}
            </div>
            <div className='modal__text' style={{border: `10px solid ${background}`}}>
              {children}
            </div>
