...
const OctogonContent = ({ showModal, onClick, projects }) => {
  ...
  function intervalMovementStart() {
    void setTimer(
      setInterval(
        () => set((state) => (state + 1) % (projects.length - 1)),
        2000
      )
    );
  }
  ...
  return (
    ...
    <div
      onClick={() => onClick({ type: "LOAD_PROJECT", index: index })}
      onMouseMove={() => clearInterval(timer)}
      onMouseLeave={() => (!showModal ? intervalMovementStart() : null)}
      style={{ height: "100%" }}
    >
